#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-dotnet-completion — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `zsh-dotnet-completion.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='zsh-dotnet-completion.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/zsh-dotnet-completion.plugin.zsh"
    assert $state equals 0
}
